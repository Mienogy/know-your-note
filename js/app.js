...
async function saveModalNote() {
    ...
    isSaving = true;
    saveModalNoteBtn.innerHTML = '<div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto"></div>';
...
function subscribeToNotes(userId) {
    ...
    unsubscribeNotes = onSnapshot(q, (snapshot) => {
        ...
        notes.forEach((note) => {
            ...
            const contentPreview = plainText.substring(0, 120) || (note.checklist?.length ? `${note.checklist.length} checklist item(s)` : 'No content');
            
            html += `
                <div class="note-card ${note.pinned ? 'pinned' : ''} p-5 group cursor-pointer" onclick="window.openNoteModal(${JSON.stringify(note).replace(/</g, '\\u003c')})">
                    <div class="flex justify-between items-start mb-3">
                        <h3 class="font-semibold text-gray-900 flex-1 text-lg pr-2 line-clamp-1">${escapeHtml(note.title) || 'Untitled'}</h3>
                        <button class="opacity-0 group-hover:opacity-100 w-6 h-6 bg-red-100 rounded-full flex items-center justify-center hover:bg-red-200 transition-colors flex-shrink-0" onclick="event.stopPropagation(); window.deleteNote('${note.id}')">
                            <i data-lucide="trash-2" class="w-3 h-3 text-red-600"></i>
                        </button>
                    </div>
                    <p class="text-sm text-gray-600 line-clamp-2 mb-4">${escapeHtml(contentPreview)}</p>
...
function renderAuthUI(user) {
    if (user) {
        ...
        notesGrid.innerHTML = '<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5"></div>';
...
document.getElementById('loginBtn')?.addEventListener('click', async () => {
    ...
    await signInWithEmailAndPassword(auth, email, pwd).catch(err => showToast(err.message, "error"));
});
...
document.getElementById('signupBtn')?.addEventListener('click', async () => {
    ...
    await createUserWithEmailAndPassword(auth, email, pwd).catch(err => showToast(err.message, "error"));
});
